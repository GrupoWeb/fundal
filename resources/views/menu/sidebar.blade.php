<div class="sidebar">
    <nav class="sidebar-nav">
      <ul class="nav">
        <li class="nav-item">
          <a class="nav-link" href="/"><i class="icon-speedometer"></i> Home </a>
        </li>
  
        <li class="nav-title">
          Menú Fundal
        </li>
        {{-- <a class="nav-link" href="/sample/dashboard"><i class="icon-calculator"></i> Samples</a> --}}
        <li class="nav-item">
          <a class="nav-link" href="estudiante"><i class="icon-calculator"></i> Estudiantes</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="jornada"><i class="icon-calculator"></i> Jornada</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="Cpadres"><i class="icon-calculator"></i> Capacitación Padres</a>
        </li>
        
        <li class="nav-item">
          <a class="nav-link" href="impartidas"><i class="icon-calculator"></i> Impartida</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="recibida"><i class="icon-calculator"></i> Capacitación Recibida</a>
        </li>
      </ul>
    </nav>
    <button class="sidebar-minimizer brand-minimizer" type="button"></button>
  </div>
  